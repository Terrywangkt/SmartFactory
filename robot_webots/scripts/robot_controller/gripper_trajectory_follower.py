...
class GripperTrajectoryFollower(object):
    ...
    def __init__(self, robot, jointStatePublisher, jointPrefix, nodeName, goal_time_tolerance=None):
        ...
        self.prefixedJointNames = [s + self.jointPrefix for s in GripperTrajectoryFollower.jointNames]
        self.jointStatePublisher = jointStatePublisher
        self.timestep = int(robot.getBasicTimeStep())
        self.motors = []
        self.sensors = []
        self.setTarget = False
        for name in GripperTrajectoryFollower.jointNames:
        ...
        self.joint_goal_tolerances = [0.05, 0.05]
        self.server = actionlib.ActionServer(nodeName + "/gripper_group_controller/follow_joint_trajectory",
    ...
    def on_cancel(self, goal_handle):
        ...
        if goal_handle == self.goal_handle:
            # stop the motors
            for i in range(len(GripperTrajectoryFollower.jointNames)):
    ...
    def update(self):
        if self.robot and self.trajectory:
            # get the last the point, self.trajectory ponts[-1]
            # assigned to controller, nod middle point
            setpoint = self.trajectory.points[-1]
